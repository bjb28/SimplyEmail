...
class ClassName(object):
    def __init__(self, Domain, verbose=False):
        ...
        try:
            ...
            self.logger.critical("GoogleXlsxSearch module failed to load: " + str(e))
            print helpers.color(
    ...
    def search(self):
        ...
        while self.Counter <= self.Limit and self.Counter <= 100:
            ...
            if self.verbose:
                ...
                self.logger.info("Google XLSX Search on page: " + str(self.Counter))
            ...
            try:
                ...
                self.logger.error("GoogleXlsxSearch failed to build url: " + str(e))
        ...
        try:
            for url in self.urlList:
                if self.verbose:
                    ...
                    self.logger.info("Google XLSX search downloading: " + str(url))
                    print helpers.color(p, firewall=True)
                try:
                    filetype = ".xlsx"
                    ...
                    if FileDownload:
                        if self.verbose:
                            p = " [*] Google XLSX file was downloaded: " + str(url)
                            self.logger.info(
                    ...
                    print helpers.color(
                        " [!] Issue with opening Xlsx Files\n", firewall=True
                    )
                    self.logger.error("Google XLSX had issue opening file")
                try:
                    dl.delete_file(FileName)
                except Exception as e:
                    self.logger.error("Google XLSX failed to delete file: " + str(e))
        except Exception as e:
            print helpers.color(
                " [*] No XLSX's to download from google!\n", firewall=True
            )
